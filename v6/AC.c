...
char local_emergencia_global[40];
...
int main(){
	...
	while (1){
		...
		serverAddr2.sin_port = htons(8001);
		...
		while(1){
			...
			// OPERAÇÃO 3 - EMERGÊNCIA --------------------------------------------------------------------------------------------------------------------
			if (operacao[0]=='4'){
				char localEmergencia[40];
				nBytes = recvfrom(udpSocket,localEmergencia,40,0,(struct sockaddr *)&serverStorage, &addr_size);
				fflush(stdin);
				//sendto(udpEmergencia, "emergencia", strlen("emergencia"), 0, (struct sockaddr *)&serverAddr2, sizeof(serverAddr2));
				printf("Local da ocorrência: %s\n",localEmergencia);
				strcpy(local_emergencia_global, localEmergencia); 
				/*
				* Recebido o sinal de emergência + local;
				* Servidor (AC) tem agora de comunicar isso à AAS
				*/
				int nBytes_send = strlen("3")+1;
				// Enviar no socket da AAS

				sendto(udpEmergencia, localEmergencia, strlen(localEmergencia), 0, (struct sockaddr *)&serverAddr2, sizeof(serverAddr2));
				fflush(stdout);
				sleep(3);
			}
 ...
 void process_client_AAS(int client_fd){
	 ...
	 while(1){
		...
		if (operacao[0]=='4'){
			header("AAS", "EMERGENCIA");
			/**
			 * Ler local de emergência proveniente de APS
			 * Passar flag e local de emergência para AAS 
			 */
			write(client_fd, "emergencia", strlen("emergencia"));
			fflush(stdout);
			write(client_fd, local_emergencia_global, strlen(local_emergencia_global));
			fflush(stdout);
		}
		 ...
		 if(operacao[0] == '7'){
			 ...
			 printf("\n\nLista de crimes:\n");
			 char c, string_crimes[1000][300]={};
			 size_t len = 0;
			 int crime_count = 0;
			 while (fgets(string_crimes[crime_count], 300,fcrimes)!=NULL){
				 fflush(stdin);
				 puts(string_crimes[crime_count]);
				 //string_crimes[crime_count][strcspn(crimes[crime_count],"\n")]=0;
				 printf("sizeof(sintrgcirmes)=%d", strlen(string_crimes[crime_count]));
				 write(client_fd, string_crimes[crime_count], strlen(string_crimes[crime_count]));
				 fflush(stdout);
				 sleep(0.5);
				 crime_count++;
			 }
		 ...
		 //OPERAÇÃO 10 - Marcar CRIME como assistido
		 if(operacao[0] == '5'){
			char arrayCrimes[40][1000];
			nread = read(client_fd, arrayCrimes, 40*1000);
			fflush(stdin);
			...
			printf("\n\nCrime marcado como assistido com sucesso!\n");
			fclose(fcrimes);
			sleep(3);
		 }
	 }
